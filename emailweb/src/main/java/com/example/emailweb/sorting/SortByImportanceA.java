...
public class SortByImportanceA implements Sorter{
    @Override
    ...
    public void QuickSort(ArrayList<Email> emails, int left, int right){
        ...
        if (left < right){
            ...
            j = right;
            while (i < j){
                while (emails.get(i).getImportance() < emails.get(pivot).getImportance() && i < right)
                    i += 1;
                while (emails.get(j).getImportance() >= emails.get(pivot).getImportance() && j > left)
                    j -= 1;
                if (i < j){
                    m = emails.get(i);
